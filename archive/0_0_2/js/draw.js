...
export function clearWaveform(canvas) {
    ...
    drawWaveformMessage(ctx, canvas.width, canvas.height, "signal: no data");
}
...
export function drawOverlay(canvas, face) {
    ...
    if (face.faceBox) {
        drawMirroredBox(ctx, width, face.faceBox, {
            strokeStyle: "#00bcd4",
            lineWidth: 2,
            label: "Face"
        });
    ...
    if (face.roiBox) {
        drawMirroredBox(ctx, width, face.roiBox, {
            strokeStyle: "#4caf50",
            lineWidth: 2,
            label: "ROI"
        });
    ...
    if (Array.isArray(face.landmarks) && face.landmarks.length > 0) {
        drawMirroredLandmarks(ctx, width, face.landmarks);
    }
}
...
function drawMirroredBox(ctx, canvasWidth, box, options = {}) {
    const mirroredBox = {
    ...
    drawBox(ctx, mirroredBox, options);
}
...
function drawMirroredLandmarks(ctx, canvasWidth, landmarks) {
    ctx.save();
    ...
    for (const point of landmarks) {
        ...
        const mirroredX = canvasWidth - point.x;

        ctx.beginPath();
        ctx.arc(mirroredX, point.y, 2, 0, Math.PI * 2);
        ctx.fill();
    }
